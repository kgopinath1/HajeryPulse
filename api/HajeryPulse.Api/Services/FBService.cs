...
public sealed class FBService : IFBService
{
    ...
    public Task<FBSummaryDto> GetSummary(string d, string st, string? sid)
        => _cache.GetOrSetAsync(K("summary", d, st, sid ?? "_"), _ttl, () => _repo.GetSummary(d, st, sid))!;

    public async Task<IEnumerable<FBBrandDto>> GetBrandSummary(string d, string st, string? sid)
        => (await _cache.GetOrSetAsync(K("bsummary", d, st, sid ?? "_"), _ttl, async () => (await _repo.GetBrandSummary(d, st, sid)).ToList())) ?? Enumerable.Empty<FBBrandDto>();
